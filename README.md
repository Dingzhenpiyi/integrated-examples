...
10. 实现了除 V2Ray/Xray 的 mKCP 应用及 Hysteria 应用（NaiveProxy 已启用 HTTP/3）之外，其它应用对外都使用 443 端口，各应用互不影响。
11. 实现了除 V2Ray/Xray 的 mKCP 应用及 Hysteria 应用（NaiveProxy 已启用 HTTP/3）之外，其它应用都支持流量伪装与防探测，且提供流量伪装与防探测的回落或代理网站都支持 HTTP 自动跳转到 HTTPS，SSL/TLS 安全评估报告为 A+（非 AES 算法的密码套件配置示例除外） 等，即所有特征完全与真实网站一致。
